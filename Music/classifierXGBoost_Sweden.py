...
def plot_feature_importance(model, name):
    ...
    if not os.path.exists('figures'):
        os.makedirs('figures')
    importance_plot.figure.savefig('./figures/Sweden/importance_plot_' + name + '.png', bbox_inches='tight')
    plt.clf()


def plot_confusion_matrix(y, y_pred, name):
    confmatr    = metrics.confusion_matrix(y, y_pred)
    matrix      = pandas.DataFrame(confmatr, index=["ESC", "MF"], columns=["ESC","MF"])
    ...
    plot_matrix.figure.savefig('./figures/Sweden/confusion_matrix_' + name + '.png')
    plt.clf()
...
def get_most_frequent(tup):
    count = {'ESC' : 0, 'MelodiFestivalen' : 0}
    for t in tup:
        count[t] += 1
...
if not os.path.exists('figures'):
        os.makedirs('figures')
if not os.path.exists('figures/Sweden'):
    os.makedirs('figures/Sweden')

data = pandas.read_csv('./files/features_music_extractor.csv')
data_Sweden = pandas.concat([data.iloc[:448,:], data.iloc[689:,:]])

data_num = data_Sweden.select_dtypes(include='number').iloc[:,1:]
...
importance_plot_top.figure.savefig('./figures/Sweden/importance_plot_total_top.png', bbox_inches='tight')
plt.clf()
